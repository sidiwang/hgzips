...
ZINB_two_gamma_optim = function(alpha1, beta1, alpha2, beta2, pi, omega, N, E, iteration, Loglik){
  ...
  for (i in c(1:N.EM)) {
    ...
    ZNegBin = function(parameter, u, N, E, Ts){
      alpha1 = parameter[1]
      beta1 = parameter[2]
      alpha2 = parameter[3]
      beta2 = parameter[4]
      ...
      max = sum(Ts *(u*(log(omega + (1 - omega)*(pi*exp(dnbinom(0, size = alpha1, prob = beta1/(E + beta1), log = TRUE))))) + (1 - u)*(log(pi) + log(1 - omega) + dnbinom(N, size = alpha1, prob = beta1/(E + beta1), log = TRUE)))) +
        sum((1 - Ts) *(u*(log(omega + (1 - omega)*((1 - pi)*exp(dnbinom(0, size = alpha2, prob = beta2/(E + beta2), log = TRUE))))) + (1 - u)*(log(1 - pi) + log(1 - omega) + dnbinom(N, size = alpha2, prob = beta2/(E + beta2), log = TRUE))))

      return(-max)
    }

    theta_EM[i + 1, 1:6] = optim(c(theta_EM$alpha1[i], theta_EM$beta1[i], theta_EM$alpha2[i], theta_EM$beta2[i], theta_EM$pi[i], theta_EM$omega[i]), ZNegBin, method = "BFGS", u = u_ij, N = N, E = E, Ts = T_ij)$par
