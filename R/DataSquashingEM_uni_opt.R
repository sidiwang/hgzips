...
source("DataSquashingEM_uni_opt.R")
EM_result2 = DSMGPS_uniroot_optimization(alpha.par = c(0.2, 2), beta.par = c(0.1, 4), pi.par = c(1/3, 1 - 1/3), N = squashed$N, squashed$E, weight = squashed$weight, iterations = 50, Loglik = TRUE)
alpha.vec = EM_result2$alpha
alpha.vec
beta.vec = EM_result2$beta
beta.vec
pi.vec = EM_result2$pi
pi.vec
Loglik = EM_result2$loglik
